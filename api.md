...
![Snip20160307_1](images/Snip20160307_1.png)￼

服务器端应该检测 API 请求版本，若 Request headers 没有 `x-api-version` 应该拒绝请求，版本错误应进行相应处理。

ps: 使用HTTP Header时，优先使用合适的标准头属性。用X-作为前缀自定义一个头属性，例如: `X-Result-Fields`。图为小写的 `x-api-version` 不规范。
